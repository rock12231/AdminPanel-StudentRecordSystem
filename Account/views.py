...
class HomeView(LoginRequiredMixin,View):
    ...
    def get(self, request):
        ...
        for i in range(len(OneMonthStudent)):
            OneMonthStudent[i]['created_at'] = OneMonthStudent[i]['created_at'].strftime("%d-%m-%Y %H:%M:%S")
            OneMonthStudent[i]['updated_at'] = OneMonthStudent[i]['updated_at'].strftime("%d-%m-%Y %H:%M:%S")
            OneMonthStudent[i]['student_attandance'] = "Present" if OneMonthStudent[i]['student_attandance'] else "Absent"
        ...
        context = {
            ...
            "start":start_date,
            "last":last_date,
            "data" :[count_list,[i for i in range(1,30)]],
    ...
    def post(self, request):
        ...
        try:
            data = User.objects.create(username=name, email=email, first_name=first_name, last_name=last_name, password="12345678")
            data.save()
